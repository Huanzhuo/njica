...
class MultiLevelExtractionICA(FastbssBasic):
    ...
    def newton_iteration_auto_break(self, B, X, max_iter, tol, break_coef):
        '''
        # newton_iteration_auto_break(self, B, X, max_iter, tol, break_coef):
        ...
        # Output:
            ...
            B: Separation matrix B.
            lim: Convergence of the iteration.
            break_by_tol: True if the Newton iteration triggers a fast break.
        '''
        _sum = 0
        _max = 0
        break_by_tol = False
        ...
        return (B, lim, break_by_tol)

    def multi_level_extraction_newton_iteration(self, X, B, max_iter, tol,
            break_coef, ext_multi_ica):
        '''
        # multi_level_extraction_newton_iteration
        # (self, X, B, max_iter, tol, break_coef, ext_multi_ica):

        # Usage:

            Newton iteration with multi-level signal extraction, the extraction
            interval is (ext_multi_ica)^grad, grad=ext_multi_ica,...,3,2,1.
        ...
        # Parameters:
            ...
            break_coef: The paramter, which determine when the iteration
                should jump out.
            ext_multi_ica: The maximum signal extraction interval is  m/((ext_multi_ica)^grad) >= n
        ...
        '''
        n, m = X.shape
        _grad = int(math.log(m//n, ext_multi_ica))
        _prop_series = ext_multi_ica**np.arange(_grad, -1, -1)
        for i in range(1, _grad+1):
            _X = X[:, ::_prop_series[i]]
            _X, V, V_inv = self.whiten_with_inv_V(_X)
            B = self.decorrelation(np.dot(B, V_inv))
            self.Stack = []
            B, _, break_by_tol = self.newton_iteration_auto_break(
    ...
    def meica(self, X, max_iter=100, tol=1e-04, break_coef=0.9, ext_multi_ica=8):
        ...
        # Usage:
            ...
            Newton iteration with multi-level signal extraction, the extraction
            interval is 2^n, n=ext_multi_ica,...,3,2,1.
        ...
        # Parameters:
            ...
            break_coef: The paramter, which determine when the iteration
                should jump out.
            ext_multi_ica: The maximum signal extraction interval is  2^ext_multi_ica
    ...
    # Following methods use naming convention (of the paper) of X, uX (mu X), W,
    # uW (muW) instead of the B of previous methods.

    def meica_generate_uxs(self, X, ext_multi_ica=2) -> list():
        """Generate a list of uX for distributed MEICA iterations.

        :param X: Mixed signals, which is obtained from the observers.
        :param ext_multi_ica: The maximum signal extraction interval is  2^ext_multi_ica
    ...
    def meica_dist_get_uw(self, uX, uW, max_iter=100, tol=1e-04, break_coef=0.9) -> tuple:
        """Perform uW iteration for distributed MEICA.
        This method takes the current uX and uW of the previous distributed
        MEICA iteration as input and perform one iteration.

        :param uX: Extracted X with ext_multi_ica.
        :param uW: Calculated W from previous MEICA step.
        :param max_iter: Maximum number of iteration.
        :param break_coef: The paramter, which determine when the iteration should jump out.

        :return:
            uW_next: uW for the next iteration.
            break_by_tol: True if the Newton iteration triggers a fast break.
        """
        self.Stack = []
        _X = uX
        _X, V, V_inv = self.whiten_with_inv_V(_X)
        uW_next = self.decorrelation(np.dot(uW, V_inv))
        self.Stack = []
        uW_next, _, break_by_tol = self.newton_iteration_auto_break(
            uW_next, _X, max_iter, tol, break_coef)
        uW_next = np.dot(uW_next, V)
        return (uW_next, break_by_tol)

    def meica_dist_get_hat_s(self, uW, X):
        """Calculate the hat_S.

        :param uW: The uW of last iteration of meica_dist_get_uw method.
        :param X: Mixed signals, which is obtained from the observers. 

        :return: Calculated hat_S.
        """
        hat_S = np.dot(uW, X)
        return hat_S
