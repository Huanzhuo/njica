...
class PyFastbssTestbed:
    ...
    def wavs_to_matrix_S(self, folder_address, duration, source_number, fixed_sources=False):
        '''
        # wavs_to_matrix_S(self, folder_address, duration, source_number):
        ...
        fixed_indexs = list(range(source_number))
        S = []
        for i in range(source_number):
            if fixed_indexs:
                sample_rate, _s = wavfile.read(wav_filenames[fixed_indexs[i]])
            else:
    ...
    def generate_matrix_S_A_X_fixed_sources(self, folder_address, wav_range, source_number, mixing_type="random", max_min=(1, 0.01), mu_sigma=(0, 1)):
        """
        Temporary attempts to make the computational latency more stable.
        The choice of source files are fixed: Always the first n files in the folder_address where n = source_number.
        """
        S = self.wavs_to_matrix_S(folder_address, wav_range, source_number, fixed_sources=True)
        A = self.generate_matrix_A(S, mixing_type, max_min, mu_sigma)
        X = np.dot(A, S)
        return S, A, X
